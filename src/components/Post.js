...
const Post = () => {
  ...
  <main className="dark-theme min-h-screen p-10">
    <section className="container mx-auto">
      <header>
        <h1 className="title-style popout-effect inline-block text-5xl text-left font-semibold pt-6 mb-12 mt-10">Articles</h1>
      </header>
      ...
      { postData && postData.map((post, index) => (
        <article key={index}>
          <Link to={"/post/" + post.slug.current} key={post.slug.current}>
            <section className="med-theme border-style block h-auto relative shadow-xl leading-snug p-4 rounded" key={index}>
